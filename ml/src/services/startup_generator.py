"""src/services/startup_generator.py
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Сервис для генерации профилей стартапов с использованием LLM.
"""
    ...
    for attempt in range(3):  # Максимум 3 попытки
        try:
            ...
            if data["niche"] != niche_name:
                logger.warning(f"Исправляем значение niche с '{data['niche']}' на '{niche_name}'")
            ...
            if niche_name == "SpaceTech":
                ...
                if not any(keyword in data["description"].lower() for keyword in space_tech_keywords):
                    logger.warning("Предупреждение: описание может не полностью соответствовать нише SpaceTech, но продолжаем...")
