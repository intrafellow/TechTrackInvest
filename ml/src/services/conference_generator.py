...
def generate_conference(request: GenerateConferenceRequest) -> ConferenceProfile:
    ...
    try:
        pipe = _get_generator()
        niche_name = request.niche if request.niche else random.choice(list(NICHES.values()))
        logger.info(f"Генерация конференции для ниши: {niche_name}")
        ...
        prompt = (
            ...
            "- nicheId: Должно быть \"{niche_name}\" (КЛЮЧ ОБЯЗАН НАЗЫВАТЬСЯ nicheId, И НИКАК ИНАЧЕ)\n"
        ...
        for attempt in range(3):  # Максимум 3 попытки
            try:
                ...
                # Перед созданием ConferenceProfile
                logger.info("Подготовка данных для создания ConferenceProfile:")
                logger.info(json.dumps(data, ensure_ascii=False, indent=2))
                
                try:
                    profile = ConferenceProfile(**data)
                    logger.info("ConferenceProfile успешно создан")
                    return profile
                except Exception as e:
                    logger.error(f"Ошибка при создании ConferenceProfile: {e}")
                    logger.error("Данные, вызвавшие ошибку:")
                    logger.error(json.dumps(data, ensure_ascii=False, indent=2))
                    raise
        ...
        logger.error(f"Критическая ошибка в generate_conference: {e}")
        raise
